...
def get_data(cadence, start, end):
    ...
    A1 =shaping_data(Waterfall(cadence[0], f_start=start, f_stop=end, max_load=20).data)
    A1 = downscale_local_mean(A1, (1, 1,FACTOR, 1,))
    num_samples = A1.shape[2]//(2048//FACTOR)
    snr = []
    # temp = np.sum(A1, axis=0)
    B =shaping_data( Waterfall(cadence[1], f_start=start, f_stop=end, max_load=20).data)
    B = downscale_local_mean(B, (1, 1,FACTOR, 1,))
    A2 =shaping_data( Waterfall(cadence[2], f_start=start, f_stop=end, max_load=20).data)
    A2 = downscale_local_mean(A2, (1, 1,FACTOR, 1,))
    C = shaping_data(Waterfall(cadence[3], f_start=start, f_stop=end, max_load=20).data)
    C = downscale_local_mean(C, (1, 1,FACTOR, 1,))
    A3 = shaping_data(Waterfall(cadence[4], f_start=start, f_stop=end, max_load=20).data)
    A3 = downscale_local_mean(A3, (1, 1,FACTOR, 1,))
    D = shaping_data(Waterfall(cadence[5], f_start=start, f_stop=end, max_load=20).data)
